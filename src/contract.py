...
def get_w3():
    print(f'connecting to {settings.GANACHE_URL}', file=sys.stdout)
    return Web3(Web3.HTTPProvider(settings.GANACHE_URL))
...
class ContractManager:
    ...
    def get_contract(self, address: str, abi_path: str):
        ...
        """
        with open(os.path.join(settings.ROOT_DIR, abi_path), 'r') as abi:
    ...
    def deploy(self, bytecode_path: str, abi_path: str):
        ...
        with open(os.path.join(settings.ROOT_DIR, bytecode_path), 'r') as bytecode:
            with open(os.path.join(settings.ROOT_DIR, abi_path), 'r') as abi:
